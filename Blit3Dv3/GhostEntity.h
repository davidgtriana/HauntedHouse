...
public:
	...
	void draw(){
		auto position = this->getComponent<dago::Position>();
		auto anim = this->getComponent<dago::AnimatedTexture>();
		anim->sprites->at(anim->frameNumber)->Blit((position->x * TileMap::TILE_SIZE) + (TileMap::TILE_SIZE / 2), blit3D->screenHeight - (position->y * TileMap::TILE_SIZE) - (TileMap::TILE_SIZE/2) );
	}
